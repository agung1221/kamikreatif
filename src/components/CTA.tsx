    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-md m-4 relative animate-modal-enter" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div><label htmlFor="service" className="block text-sm font-medium text-slate-700 mb-1">Topik Diskusi</label><select id="service" name="service" value={formData.service} onChange={handleChange} className="w-full px-4 py-2 border border-slate-300 rounded-md focus:ring-orange-500 focus:border-orange-500"><option>Diskusi Umum</option><option>Paket Basic</option><option>Paket Pro</option><option>Paket Custom</option></select></div>
